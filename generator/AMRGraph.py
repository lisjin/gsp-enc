...
class AMRGraph(object):
    ...
    def collect_relations(self, g, nodes):
        relations = dict()
    ...
    def collect_edges(self, g, nodes):
        relations = dict()
        v2i = {v: i for i, v in enumerate(nodes)}
        for src in nodes:
            i = v2i[src]
            relations[i] = dict()
            for tgt in g.adj[src].keys():
                j = v2i[tgt]
                info = dict()
                info['edge'] = [g[src][tgt]['label']]
                info['length'] = 1
                relations[i][j] = [info]
        return relations
    ...
    def collect_concepts_and_relations(self):
        ...
        relations = self.collect_edges(g, nodes)
